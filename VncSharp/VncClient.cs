...
namespace VncSharp
{
	...
	public class VncClient
	{
        ...
        private System.Timers.Timer RfbUpdatesWatchdog; // System.Timers.Timer is ThreadSafe

        private VncClientWatchdog watchdog = new VncClientWatchdog();
		...
		public VncClient(int ConnectTimeout)
		{
            TimeoutConnect = ConnectTimeout;
            RfbUpdatesWatchdog = new System.Timers.Timer(3333);
            RfbUpdatesWatchdog.Elapsed += new System.Timers.ElapsedEventHandler(this.RfbUpdatesTimerElapsed);
		}
        ...
        public bool SocketConnected
        {
            get
            {
                if (rfb!=null) {return rfb.SocketConnected; }
                return false;
            }
        }
		/// <summary>
		...
		public bool Connect(string host, int display, int port, bool viewOnly)
		{
            ...
            if (this.SocketConnected) throw new ArgumentException("Already connected.");
			...
			try
            {
                if (log.IsInfoEnabled) log.Info("VncClient Connect(" + host + ":" + port + ") Timeout " + TimeoutConnect + " ms");
				rfb.Connect(host, port,TimeoutConnect);
                if (log.IsInfoEnabled) log.Info("VncClient ConnectB " + host + ":" + port + ") Timeout " + TimeoutConnect + " ms");
                rfb.ReadProtocolVersion();
				rfb.WriteProtocolVersion();
                if (log.IsInfoEnabled) log.Info("VncClient ConnectC " + host + ":" + port + ") Timeout " + TimeoutConnect + " ms");
				...
				if (types.Length > 0)
                {
                    ...
                    else
                    {
						...
						if (!(securityType >0))
                        {
                            if (log.IsErrorEnabled) log.Error("The server sent one or more unknown Security Types.");
                        }
            ...
            {
				throw new VncProtocolException(e.Message, e);
			}
		}
...
//					|| types[i] == 18 	// TLS
		...
		public bool Authenticate(string password)
		{
            if (password == null)
            {
                //log.Error("Authenticate failed. No password supplied");
            ...
            {
                //log.Error("Unable to Authenticate with Server. The Server uses an Authentication scheme unknown to the client.");				
            ...
            else 
            {
                ...
                if (log.IsDebugEnabled) log.Debug("VncClient.Authenticate() closing rfb");
		...
		{
            if (log.IsDebugEnabled) log.Debug("VncClient.Disconnect() Closing rfb");
            this.RfbUpdatesWatchdog.Stop();
		...
		private void GetRfbUpdates()
		{
			...
			while (true)
            {
                ...
                try {
                    switch (rfb.ReadServerMessageType()) 
                    {
                        case RfbProtocol.FRAMEBUFFER_UPDATE:
                            ...
                            this.watchdog.DataReceived = true;
                ...
                {
                    if (log.IsDebugEnabled) log.Debug("VncClient - GetRfbUpdates: " + ex.Message);
                    OnConnectionLost();
                }
			}
		}
		...
		protected void OnConnectionLost()
		{
			// In order to play nicely with WinForms controls, we do a check here to 
			// see if it is necessary to synchronize this event with the UI thread.
			if (ConnectionLost != null && ConnectionLost.Target is System.Windows.Forms.Control) 
            {
                ...
                {
                    if (log.IsDebugEnabled) log.Debug("VncClient raising ConnectionLost");
...
// There is no managed way to get a system beep (until Framework v.2.0). So depending on the platform, something external has to be called.
        ...
        public void RequestLifeSignFrame()
        {
            if (rfb != null)
            {
                if (rfb.SocketConnected)
                {
                    try
                    {
                        // Request area x,y = 0,0 and w,h = 8,8. 
                        // incremental update false will force the server to send FramebufferUpdate.
                        rfb.WriteFramebufferUpdateRequest(0, 0, (ushort)8, (ushort)8, false);
                        if (log.IsDebugEnabled) log.Debug("RequestLifeSign RequestLifeSignFrame");
                    }
                    catch (Exception ex)
                    {
                        // catch a lost connection
                        if (log.IsErrorEnabled) log.Error("RequestLifeSign " + ex.Message);
                        // note: RemoteDesktop event handler will call vnc.Disconnect()
                        OnConnectionLost();
                    }
                }
            }
        ...
        private void RfbUpdatesTimerElapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            if (rfb != null)
            {
                if (rfb.SocketConnected)
                {
                    if (this.watchdog.SendLifeSign)
                    {
                        RequestLifeSignFrame();
                    }
                }
                else
                {
                    //if (log.IsDebugEnabled) log.Debug("RequestLifeSign rfb.Socket not connected");
                }
            }
            else
            {
                if (log.IsDebugEnabled) log.Debug("RequestLifeSign rfb null");
            }
        }
	}
}