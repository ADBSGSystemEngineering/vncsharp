	...
	public sealed class VncDesignModeDesktopPolicy : VncDesktopTransformPolicy
	{
        ...
        public override Rectangle RepositionImage(Image desktopImage)
        {
			...
			if (remoteDesktop.ClientSize.Width > desktopImage.Width)
            {
                ...
                if (log.IsDebugEnabled) log.Debug("RepositionImage: remoteDesktop.ClientSize.Width " + remoteDesktop.ClientSize.Width + " > desktopImage.Width "+desktopImage.Width + " => " + x);
			}
            else
            {
			...
			if (remoteDesktop.ClientSize.Height > desktopImage.Height )
            {
                ...
                if (log.IsDebugEnabled) log.Debug("RepositionImage: remoteDesktop.ClientSize.Height " + remoteDesktop.ClientSize.Height + " > desktopImage.Height " + desktopImage.Height+" => "+y);
			}
            else
            {
