...
namespace VncSharp
{
	...
	public class RfbProtocol
	{
        ...
        public static ManualResetEvent CancelConnecting = null;
		...
		public void Connect(string host, int port, int timeoutSeconds)
		{
            ...
            if (log.IsDebugEnabled) log.Debug("RfbProtocol.Connect() tcp BeginConnect(" + host + ") timeout " + timeoutSeconds);
            Stopwatch sw = new Stopwatch();
            CancelConnecting = new ManualResetEvent(false);
            sw.Start();
            IAsyncResult result = tcp.BeginConnect(host, port, null, null);
            WaitHandle[] ConnectedTimedOutOrCanceled = new WaitHandle[] { CancelConnecting, result.AsyncWaitHandle };
            WaitHandle.WaitAny(ConnectedTimedOutOrCanceled,TimeSpan.FromSeconds(timeoutSeconds));
            sw.Stop();            
            if (tcp.Connected)
            {
                if (log.IsDebugEnabled) log.Debug("RfbProtocol.Connect() tcp Connected in " + sw.Elapsed.Seconds + " seconds");
                ...
                if (log.IsDebugEnabled) log.Debug("RfbProtocol.Connect() created new reader & writer in " + sw.Elapsed.Seconds + " seconds");
            }
            else
            {
                if (sw.Elapsed.Seconds == 0)
                {
                    throw new ArgumentException("RfbProtocol.Connect() tcp cannot connect (" + host + " not reachable)");
                }
                else
                {
                    //if (log.IsErrorEnabled) log.Error("tcp not Connected...");
                    throw new ArgumentException("RfbProtocol.Connect() tcp not connected after " + sw.Elapsed.Seconds + " seconds");
                }
            }
        }
        ...
        public void Close()
		{
            if (CancelConnecting != null)
            {
                CancelConnecting.Set();
            }
			try
            {
                if (log.IsDebugEnabled) log.Debug("RfbProtocol.Close() closing reader & writer");
				writer.Close();
				reader.Close();
				stream.Close();
				tcp.Close();
		...
		public void WriteProtocolVersion()
		{
            ...
            Debug.Assert(verMinor == 3 || verMinor == 7 || verMinor == 8, "Wrong Protocol Version!",
                         ...
                         */
            if (verMinor != 3 && verMinor != 7 && verMinor != 8)
            {
                if (log.IsErrorEnabled) log.Error("Protocol Version should be 3.3, 3.7, or 3.8 but is "+ verMajor.ToString()+"."+ verMinor.ToString());
            }
		...
		public byte[] ReadSecurityTypes()
		{
            ...
            else
            {
                ...
                {
                    if (log.IsErrorEnabled) log.Error("New connection ReadSecurityTypes() failed. VNC server too slow?");
                }    
			}
			return types;
		}
		...
		public void WriteSecurityType(byte type)
		{
            ...
            if (!(type >= 1))
            {
                if (log.IsErrorEnabled) log.Error("The Security Type must be one that requires authentication.");
            }
		...
		public Framebuffer ReadServerInit()
		{
            ...
            try
            {
                ...
                buffer = Framebuffer.FromPixelFormat(reader.ReadBytes(16), w, h);
                int length = (int)reader.ReadUInt32();
                buffer.DesktopName = GetString(reader.ReadBytes(length));
                if ((w == 0) || (h == 0))
                {
                    if (log.IsErrorEnabled) log.Error("RfbProtocol ReadServerInit() Framebuffer Width=" + w + " Height=" + h + " Length=" + length);
                }
                else if (log.IsDebugEnabled)
                {
                    log.Debug("RfbProtocol ReadServerInit() Framebuffer Width=" + w + " Height=" + h + " Length=" + length);
                }
            }
            catch
            {
                if (log.IsErrorEnabled) log.Error("RfbProtocol ReadServerInit()");
            }
			return buffer;
		}
