...
namespace VncSharp
{
	...
	public class EncodedRectangleFactory
	{
		...
		public EncodedRectangleFactory(RfbProtocol rfb, Framebuffer framebuffer)
		{
			...
			if (rfb==null)
            {
                if (log.IsErrorEnabled) log.Error("EncodedRectangleFactory: RfbProtocol object null");
            }
            if (framebuffer==null)
            {
                if (log.IsErrorEnabled) log.Error("EncodedRectangleFactory: framebuffer object null");
            }
            this.rfb = rfb;
